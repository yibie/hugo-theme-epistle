...
task015 [ ] 场景:维护者只管理一个私人仓库即可审核来信和维护博客源码 | Given:现有私人 inbox 仓库为空+生产 Hugo 源码仅在本机 | When:初始化源码并安装发布与部署工作流 | Then:publish 标签在同仓库创建无私人字段的 PR+合并 main 自动更新公开站点+Worker Token 无需源码权限 | 验证:模板回归测试+Hugo 干净构建+GitHub Actions 实际运行+线上页面检查
