...
| **文章往来** | 每封信下方都可匿名来信；表单说明来信可能公开，投递先进入私人收件箱并由作者择选 |
...
「往来」不是公开评论区。每篇文章的下半部分先展示作者择选公开的来信与可选回信；点击「往来」即可写信，暂无来信时则直接显示信笺。访客可以匿名投递，邮件地址只用于作者私下回复。

表单会明确提示来信可能被摘录公开，寄出即表示知悉。来信先进入保存 Hugo 源码的私人 GitHub 仓库；作者添加 `publish` 标签即完成批准，系统会直接提交不含邮箱的内容并部署，无需再合并 PR。作者可以不公开回复，公开内容只出现在它所回应的文章下方。

它由三部分组成：

- Cloudflare Worker：验证来源和 Turnstile，接收匿名投递
- 私人 GitHub Issues：保存原始来信，并通过 GitHub 通知发送收件提醒
- Hugo 发布器：将已批准的来信和可选回复生成为安全转义的静态页面

完整配置、部署、审核和回滚步骤见 [往来部署指南](docs/guestbook.md)。
